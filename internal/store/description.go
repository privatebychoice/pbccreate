...
func (d Description) Render() string {
	blocks := []string{d.Intro, d.Chapters, d.Links, d.Sponsor, d.Credits, d.Disclosure, d.Hashtags}
...
func GetDescription(ctx context.Context, db *sql.DB, contentItemID int64) (Description, error) {
	...
	err := db.QueryRowContext(ctx,
		`SELECT content_item_id, intro, chapters, links, sponsor, credits, hashtags, disclosure, created_at, updated_at
		 FROM descriptions WHERE content_item_id = ?`, contentItemID).
		Scan(&d.ContentItemID, &d.Intro, &d.Chapters, &d.Links, &d.Sponsor, &d.Credits, &d.Hashtags, &d.Disclosure, &created, &upd)
	switch {
...
func SaveDescription(ctx context.Context, db *sql.DB, d Description) (Description, error) {
	...
	_, err := db.ExecContext(ctx, `
		INSERT INTO descriptions (content_item_id, intro, chapters, links, sponsor, credits, hashtags, disclosure, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		d.ContentItemID, d.Intro, d.Chapters, d.Links, d.Sponsor, d.Credits, d.Hashtags, d.Disclosure, now, now)
	if err != nil {
