...
func CreateAttribution(ctx context.Context, db *sql.DB, a Attribution) (Attribution, error) {
	...
	res, err := db.ExecContext(ctx, `
		INSERT INTO attributions
			(content_item_id, name, kind, provider, license, license_id, credit_text, source_url, media_asset_id, included_in_description, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		nullableID(a.MediaAssetID), included, ts)
	if err != nil {
...
func ListAttributions(ctx context.Context, db *sql.DB, contentItemID int64) ([]Attribution, error) {
	rows, err := db.QueryContext(ctx, `
		SELECT id, content_item_id, name, kind, provider, license, license_id, credit_text, source_url, media_asset_id, included_in_description
		FROM attributions
		WHERE content_item_id = ?
		ORDER BY id DESC`, contentItemID)
	if err != nil {
...
func scanAttribution(sc rowScanner) (Attribution, error) {
	...
	if err := sc.Scan(&a.ID, &a.ContentItemID, &a.Name, &a.Kind, &a.Provider, &a.License,
		&a.LicenseID, &a.CreditText, &a.SourceURL, &mediaID, &a.IncludedInDescription); err != nil {
